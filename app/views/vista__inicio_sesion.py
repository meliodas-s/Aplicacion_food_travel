...
class VistaInicioSesion(customtkinter.CTkFrame):
    def __init__(self, master=None, controlador=None,
        ...
        self.boton_inicio = customtkinter.CTkButton(
            self.frame_central, text='Iniciar Sesion', command=self.controlador.comprobar_usuario, height=40, fg_color=self.c_gris_azulado, hover_color=self.c_verde)
        self.boton_inicio.grid(column=0, row=i + 2, pady=10)

        # (Boton Registrarse)
        self.boton_registrarse = customtkinter.CTkButton(
            self.frame_central, text='Registrarse', command=self.controlador.ir_registrarse, height=40, fg_color=self.c_gris_azulado, hover_color=self.c_verde)
